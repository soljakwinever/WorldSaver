...
namespace Project.Tests.EditMode
{
    public sealed class RegionSaveCodecTests
    {
        ...
        [Test]
        public void ChangedWallHealthSurvivesBinaryRoundTripAsByte()
        {
            RegionSaveData original = new()
            {
                coordinate = new Vector2Int(1, -1)
            };
            ChunkState chunk = new() { localChunkIndex = 5 };
            chunk.wallHealth.Add(new WallHealthData
            {
                localX = 12,
                localY = 23,
                health = 87
            ...
            Assert.That(restored.changedChunks[0].wallHealth, Has.Count.EqualTo(1));
            WallHealthData health = restored.changedChunks[0].wallHealth[0];
            Assert.That(health.localX, Is.EqualTo(12));
            Assert.That(health.localY, Is.EqualTo(23));
            Assert.That(health.health, Is.EqualTo(87));
        }
        ...
        [Test]
        public void ExistingIndoorCoverageIsPreservedWithoutAccumulating()
        {
            ...
            Assert.That(amount, Is.EqualTo(0.4f));
        }
        ...
        [Test]
        public void ChunkPersistenceRootStoresOnlyExplicitlyChangedWallHealth()
        {
            GameObject gameObject = new("Wall Health Persistence Test");
            try
            {
                ChunkPersistenceRoot root =
                    gameObject.AddComponent<ChunkPersistenceRoot>();
                ...
                Assert.That(root.Capture(1).wallHealth, Is.Empty);

                root.SetWallHealth(4, 9, 64);
                ChunkState damaged = root.Capture(2);
                Assert.That(damaged.wallHealth, Has.Count.EqualTo(1));
                Assert.That(damaged.wallHealth[0].health, Is.EqualTo(64));

                root.RemoveWallHealth(4, 9);
                Assert.That(root.Capture(3).wallHealth, Is.Empty);
            }
            finally
            {
                Object.DestroyImmediate(gameObject);
